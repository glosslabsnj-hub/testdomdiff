...
interface WardenTip {
  id: string;
  category: "discipline" | "workout" | "scripture" | "nutrition" | "faith";
  text: string;
}

// Pre-defined rotating tips - no AI generation, just a curated list
const WARDEN_TIPS: WardenTip[] = [
  ...
  // Workout
  { id: "w1", category: "workout", text: "Iron sharpens iron. Time to hit the yard." },
  { id: "w2", category: "workout", text: "Your body can handle more than your mind thinks. Push through." },
  { id: "w3", category: "workout", text: "Every rep is a vote for the man you're becoming." },
  { id: "w4", category: "workout", text: "Rest is earned, not given. Put in the work first." },
  { id: "w5", category: "workout", text: "Sweat today, strength tomorrow. The compound effect is real." },
...
export function useWardenTips() {
  ...
  const getNextTip = useCallback(() => {
    ...
    const availableTips = WARDEN_TIPS.filter(tip => !shownTipIds.has(tip.id));
    
    // If we've shown all tips, reset the shown list
    if (availableTips.length === 0) {
      setShownTipIds(new Set());
      const randomIndex = Math.floor(Math.random() * WARDEN_TIPS.length);
      const nextTip = WARDEN_TIPS[randomIndex];
