...
DROP POLICY IF EXISTS "Admins can manage social profile audits" ON public.social_profile_audits;
...
DROP POLICY IF EXISTS "Admins can manage social content calendar" ON public.social_content_calendar;
...
-- Social Trend Scans
CREATE TABLE IF NOT EXISTS public.social_trend_scans (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  platform text NOT NULL,
...
DROP POLICY IF EXISTS "Admins can manage social trend scans" ON public.social_trend_scans;
